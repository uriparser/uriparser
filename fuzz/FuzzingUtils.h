...
inline UriString consumeBytesAsString(FuzzedDataProvider & fdp, size_t chars) {
	UriString str(chars, 0);
	size_t bytes = fdp.ConsumeData(str.data(), chars * sizeof(URI_CHAR));
...
inline UriString consumeRemainingBytesAsString(FuzzedDataProvider & fdp) {
	...
	return consumeBytesAsString(fdp, chars);
}
